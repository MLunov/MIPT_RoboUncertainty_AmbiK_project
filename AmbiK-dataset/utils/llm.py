...
class LLM:
    def __init__(self, model_name, generation_settings):
        ...
        if "t5" in model_name or "bart" in model_name:
            ...
            self.mtype ='ed'
        elif 'turbo' in model_name or 'gpt-4' in model_name:
...
if __name__ == "__main__":
    ...
    model = LLM("google/flan-t5-large", {"max_new_tokens":1})
    # model = LLM("TheBloke/Mistral-7B-Instruct-v0.1-AWQ", {"max_new_tokens":1})
    prompt = (
        """
        ...
        We: You have received the task “Prepare a Cheesy Greek Yogurt Dip.” You created a plan to compete the task.
    ...
    answer, logits = model.generate(prompt, return_logits=True)
    
    print(f"PROMPT:\n{prompt}")
    print(f"ANSWER:\n{answer}")
    print(f"LOGITS:\n{logits}")
    print(f"filter_logits:\n{model.filter_logits(logits[-1][0], words=['A', 'B', 'C', 'D', 'a', 'b', 'c', 'd', '1', '2', '3', '4'])}")
