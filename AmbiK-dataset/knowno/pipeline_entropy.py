...
class KnowNoPipe():
    ...
    def answer_with_entropy(self, tokens_logits): 
        top_tokens_logits = dict(sorted(tokens_logits.items(), key=lambda x: -x[1])[:2])
        possible_options = []
        for key in top_tokens_logits.keys():
            possible_options.append(key) 
        formated_options = []
        for option in possible_options:
            if option.isdigit():  
    ...
    def compute_entropy(self, filtered_logits: dict):
        # Преобразуем значения логитов в список
        logits_tensor = torch.tensor(list(filtered_logits.values()), dtype=torch.float32)
    ...
    def generate_answer_batch(self, prompts, batch_size=2): #choosing CP set for batch
        ...
        for i in range(len(full_texts)):
            filtered_logits = llm.filter_logits(
              full_logits[i][0],
              words=["A", "B", "C", "D", 'a', 'b', 'c', 'd', '1', '2', '3', '4']
              )
            #print(filtered_logits)
            filtered_logits_batch.append(filtered_logits)
            # print(f"answer_with_cp: {self.answer_with_cp(filtered_logits)}")
            # Добавляем вычисление энтропии
            entropy = self.compute_entropy(filtered_logits)
            if entropy > 2.45:
                # print("High uncertainty — asking for help.")
                # top_n_answers = sorted(filtered_logits.items(), key=lambda x: -x[1])[:2]
                # answer = [x[0] for x in top_n_answers]
                
                answer = self.answer_with_entropy(filtered_logits)
                # print(f"answer with entropy: {answer}")
            else:
    ...
    def generate_answer(self, prompt, description, task, prefix, action): 
        llm = LLM(self.config.answering['model'],
                  self.config.answering['generation_kwargs'])
        ...
        # Возьмем последний логит (для последнего токена)
        last_logits = logits[-1][0]  # логиты последнего токена
        filtered_logits = llm.filter_logits(last_logits, words=["A", "B", "C", "D", 'a', 'b', 'c', 'd', '1', '2', '3', '4'])
        ...
        if entropy > 2.45:  
            print("High uncertainty — asking for help.")
            return filtered_logits, self.answer_with_entropy(filtered_logits)

            # top_n_answers = sorted(filtered_logits.items(), key=lambda x: -x[1])[:2]
            # return filtered_logits, [x[0] for x in top_n_answers]
...
if __name__ == "__main__":
    ...
    dataset = pd.read_csv("./ambik_dataset/ambik_test_400.csv").iloc[:2] #ambik_test_400.csv #ambik_test_for_testing.csv
