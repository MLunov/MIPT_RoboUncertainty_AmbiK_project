...
from knowno.pipeline_entropy import KnowNoConfig, KnowNoPipe
...
def get_logits(knowno, description, task, prefix, action): #def get_logits(knowno, prompt)
     ...
     print(f"Options generated by model:\n{options}")
     gc.collect()

     choose = knowno.generate_answer(options, description, task, prefix, action)
     print(f"Chosen answer: {choose[0]}\n\n") #Вывод логитов для отладки
     gc.collect()
     
     return options, choose[0] # Возврат всех вариантов и логитов
